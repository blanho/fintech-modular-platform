...
if (!string.IsNullOrWhiteSpace(request.IdempotencyKey))
        {
            ...
            {
                return Result<DepositResponse>.Success(MapToResponse(existingTransaction));
            }
        }
...
if (walletResult.Value!.Currency != request.Currency)
            return Result<DepositResponse>.Failure(
                Error.Validation($"Currency mismatch: wallet uses {walletResult.Value.Currency}, request uses {request.Currency}"));
...
var completeResult = transaction.Complete();
    ...
    private static DepositResponse MapToResponse(Transaction transaction)
    {
