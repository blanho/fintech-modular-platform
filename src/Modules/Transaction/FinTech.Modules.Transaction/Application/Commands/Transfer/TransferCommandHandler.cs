...
if (sourceWalletResult.Value!.Currency != request.Currency)
            return Result<TransferResponse>.Failure(
                Error.Validation($"Currency mismatch: wallet uses {sourceWalletResult.Value.Currency}, request uses {request.Currency}"));

        if (targetWalletResult.Value!.Currency != request.Currency)
            return Result<TransferResponse>.Failure(
                Error.Validation($"Target wallet currency mismatch: wallet uses {targetWalletResult.Value.Currency}, request uses {request.Currency}"));
